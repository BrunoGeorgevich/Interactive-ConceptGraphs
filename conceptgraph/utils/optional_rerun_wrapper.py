...
def orr_log_depth_image(depth_tensor):
    ...
    if len(depth_in_meters.shape) == 3 and depth_in_meters.shape[2] == 4:
        depth_in_meters = depth_in_meters[:, :, 3]
    elif len(depth_in_meters.shape) == 3 and depth_in_meters.shape[2] == 3:
        depth_in_meters = cv2.cvtColor(depth_in_meters, cv2.COLOR_BGR2GRAY)
    elif len(depth_in_meters.shape) == 3 and depth_in_meters.shape[2] == 1:
        depth_in_meters = depth_in_meters[:, :, 0]
    elif len(depth_in_meters.shape) == 2:
        pass
    else:
        raise ValueError(
            "Depth tensor has unexpected shape. Expected 3D tensor with 4, 3 or 1 channel."
        )
